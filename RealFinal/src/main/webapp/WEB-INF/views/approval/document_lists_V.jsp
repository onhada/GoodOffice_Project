...
$(document).ready(function() {
	...
	$("td.tableData").each(function(){
		$(this).click(function(){
			...
			$(location).attr('href', '<%=ctxPath%>/approval/documentDetail/list/V/view.gw?formId=' + formId + '&approvalId=' + $(this).parent().find("td.docu-num div").html());
		})
	})
...
function batchApproval(){
	...
	// 폼(form)을 전송(submit)
	  ...
	  frm.method = "post";
	  frm.action = "<%= ctxPath%>/approval/batch/check.gw";
	  frm.submit();
}
				...
				<table class="tableType01" id="tableApprovalDocument" style="width: 1604px;">
					<thead>
						<tr>
							...
							<th class="resizable-pdr-0" style="white-space: nowrap; width: 738px;">
								<div class="column-resizer ui-resizable" style="width: 738px; float: left; display: block; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">
							...
							<th style="width: 80px; white-space: nowrap;" class="resizable-false">결과</th>
						</tr>
							...
							<form name="checkFrm">
								<c:forEach var="approvalVo" items="${requestScope.ingList}">
									...
									<tr>
										...
										<td class="new-open-window resizable-pdl-0 resizable-pdr-0 tableData" data-href="수정필" id="">
											<span class="icon h_new span-new-link" style="margin-top: 0px;" data-link-url="수정필"></span>
										</td>
										...
										<td title="${approvalVo.draftDay}" class="tableData" data-href="수정필">${fn:substring(approvalVo.draftDay, 0, fn:indexOf(approvalVo.draftDay, " "))}</td>
										<td class="docu-type tableData" data-href="수정필">${approvalVo.procedureType}</td>
										<td class="docu-result tableData" data-href="수정필">
