...
public class ApprovalService_imple implements ApprovalService {
	...
	public ApprovalDetailVO getApprovalDocumentView(Map<String, Long> paraMap) {
		...
		System.out.println("//////////////////////////////////////////////////////////");
		System.out.println("content : " + advo.getContent());
		System.out.println("//////////////////////////////////////////////////////////");
				...
				if (dao.hasApplicantAndApproverAllAccept(paramMap) == 0) {
					...
					if (dao.updateApprovalComplete(paraMap) == 1) {
						...
						if("103".equals(paraMap.get("formId"))) {
							...
							// 업데이트할 근무정보를 취득한다
							ModifyWorkRequestVO mvo = dao.getModifyWorkRequest(Long.parseLong(paraMap.get("approvalId")));
							// 업데이트한다
							for(ModifyDetailVO mdvo : mvo.getModifyDetailList()) {
								mdvo.setEmpId(Long.parseLong(paraMap.get("empId")));
								
								if("수정".equals(mdvo.getRequestType())) {
									// update
								...
								}else if("추가".equals(mdvo.getRequestType())) {
									// insert
							...
							// 휴가 신청
							DayOffVO dvo = dao.getDayOffDetail(Long.parseLong(paraMap.get("approvalId")));
							
							/*
							 * if(dao.insertWorkHistory(mdvo) != 1) { return false; }
							 */
